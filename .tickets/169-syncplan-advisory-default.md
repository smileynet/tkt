...
- [ ] `plan-status-drift` is `warning`; default `sync-plan` with drift exits 0
- [ ] `sync-plan --check` exits 1 when drift exists (repurposed from dead flag)
- [ ] `sync-plan --fix --dry-run` previews changes without writing the plan file
- [ ] Non-derivable conflicts still error (id in plan with no ticket)
- [ ] Docs reconciled: advisory status documented, `docs/plan.md` vs `PLAN.md` fixed across surfaces
